...
// 1) Provider router: respect configured order and make OpenRouter the optional third fallback.
{
  ...
  if (!source.includes('async function documentContext(')) {
    const documentFn = [
      '',
  ...
  if (!source.includes('const documentIds = Array.isArray(req.body?.documentIds)')) {
  ...
  if (source.includes(promiseBlock)) {
...
// 4) Remove remaining user-facing Serena/Gemini branding while preserving internal compatibility routes.
{
