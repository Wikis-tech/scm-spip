...
async function sendPushToUser(userId: string, reminder: any) {
  if (!vapidPublic || !vapidPrivate) {
    return { delivered: 0, failed: 0, reason: 'VAPID_NOT_CONFIGURED' };
  }
  ...
  const payload = JSON.stringify({
    ...
    requireInteraction:
      reminder.priority === 'critical' || Boolean(reminder.metadata?.requireInteraction),
    timestamp: Date.now(),
  });
...
async function dispatchDue(onlyUserId?: string) {
  ...
  for (const candidate of reminders || []) {
    ...
    // Critical meeting-start alerts bypass quiet hours; lower urgency reminders wait.
    ...
    try {
      ...
      await supabase
        .from('spip_reminders')
        .update({
          last_error: clean(error?.message || 'DISPATCH_ERROR', 300),
...
async function resyncGeneratedReminders(userId: string) {
  const [{ data: meetings }, { data: tasks }] = await Promise.all([
    supabase.from('meetings').select('id,date').eq('officer_id', userId).limit(250),
    supabase.from('tasks').select('id,due_date').eq('officer_id', userId).eq('is_completed', false).limit(250),
  ]);
  for (const meeting of meetings || []) {
    await supabase.from('meetings').update({ date: meeting.date }).eq('id', meeting.id);
  }
  for (const task of tasks || []) {
    await supabase.from('tasks').update({ due_date: task.due_date }).eq('id', task.id);
  }
}
...
export async function handlePhase4(req: any, res: any, path: string): Promise<boolean> {
  if (path === 'push/public-key' && req.method === 'GET') {
    res
      .status(vapidPublic ? 200 : 503)
      .json(vapidPublic ? { publicKey: vapidPublic } : { error: 'Push notifications are not configured.' });
    return true;
  }
  ...
  if (path === 'reminders/dispatch' && req.method === 'POST') {
    ...
    const supplied = String(
      req.headers['x-spip-reminder-secret'] ||
    ...
    try {
      res.json({ ok: true, ...(await dispatchDue()) });
    } catch (error: any) {
      res.status(500).json({ error: 'Reminder dispatch failed.', detail: clean(error?.message, 300) });
    }
    return true;
  }
  ...
  if (path === 'reminders/dispatch-self' && req.method === 'POST') {
    try {
      res.json({ ok: true, ...(await dispatchDue(user.id)) });
    } catch (error: any) {
      res.status(500).json({ error: 'Reminder sync failed.', detail: clean(error?.message, 300) });
    }
    return true;
  }
  ...
  if (path === 'push/subscribe' && req.method === 'POST') {
    ...
    if (error) res.status(500).json({ error: 'Unable to register this device.' });
      ...
      await supabase.from('spip_notification_preferences').upsert(
  ...
  if (path === 'reminders' && req.method === 'GET') {
    ...
    const { data, error } = await supabase
      ...
      .order('scheduled_for', { ascending: true })
      .limit(limit);
    if (error) res.status(500).json({ error: 'Unable to load reminders.' });
    else res.json((data || []).map(mapReminder));
    return true;
  }
  ...
  if (path === 'reminders/custom' && req.method === 'POST') {
    ...
    if (!Number.isFinite(scheduled.getTime()) || scheduled.getTime() <= Date.now()) {
      res.status(400).json({ error: 'Choose a future reminder time.' });
      return true;
    }
    ...
    const { data, error } = await supabase
      ...
      .upsert(
        {
          user_id: user.id,
          source_type: req.body?.sourceType === 'follow_up' ? 'follow_up' : 'custom',
          source_id: sourceId,
          prospect_id: clean(req.body?.prospectId, 200) || null,
          prospect_name: clean(req.body?.prospectName, 200) || null,
          title,
          message: message || title,
          reminder_kind: 'custom',
          scheduled_for: scheduled.toISOString(),
          priority: ['normal', 'high', 'critical'].includes(req.body?.priority)
            ? req.body.priority
            : 'normal',
          status: 'PENDING',
          next_attempt_at: null,
          sent_at: null,
          last_error: null,
          expires_at: new Date(scheduled.getTime() + 24 * 60 * 60_000).toISOString(),
          metadata: { url: req.body?.url || '/calendar' },
          updated_at: new Date().toISOString(),
        },
        { onConflict: 'user_id,source_type,source_id,reminder_kind' },
      )
      .select('*')
      .single();
    if (error) res.status(500).json({ error: 'Unable to create reminder.' });
    else res.status(201).json(mapReminder(data));
    return true;
  }
  ...
  if (snooze && req.method === 'POST') {
    ...
    const { data, error } = await supabase
      ...
      .select('*')
      .maybeSingle();
    if (error || !data) res.status(404).json({ error: 'Reminder not found.' });
    else res.json(mapReminder(data));
    return true;
  }
  ...
  if (cancel && req.method === 'DELETE') {
    await supabase
      .from('spip_reminders')
      .update({ status: 'CANCELLED', next_attempt_at: null, updated_at: new Date().toISOString() })
      .eq('id', cancel[1])
      .eq('user_id', user.id);
    res.json({ ok: true });
    return true;
  ...
  if (path === 'notification-preferences' && req.method === 'PATCH') {
    ...
    if (error) res.status(500).json({ error: 'Unable to save notification preferences.' });
      ...
      await resyncGeneratedReminders(user.id);
      res.json(data);
    }
    return true;
  }
