...
export const IntelligenceCopilot: React.FC<IntelligenceCopilotProps> = ({ currentUser }) => {
  ...
  const refreshConversations = async () => {
    try {
      ...
      setConversations(Array.isArray(data) ? data : Array.isArray(data?.conversations) ? data.conversations : []);
    } catch {
      // Conversation list is non-blocking; chat itself may still work.
    }
  };
  ...
  useEffect(() => {
    ...
    authedFetch('/api/ai/status')
      ...
      .then((data) => setProvidersReady(Boolean(data?.ready)))
    ...
    <div className="grid min-h-[calc(100vh-8.5rem)] grid-cols-1 gap-4 lg:grid-cols-[260px_minmax(0,1fr)]">
      <aside className="hidden rounded-2xl border border-slate-200 bg-white p-3 shadow-sm lg:block">
        ...
        <div className="space-y-1">
          {conversations.length === 0 && <div className="rounded-xl bg-slate-50 p-3 text-xs text-slate-500">Your Copilot history will appear here.</div>}
          {conversations.slice(0, 30).map((conversation) => (
            <div key={conversation.id} className={`group flex items-center gap-1 rounded-xl ${conversationId === conversation.id ? 'bg-slate-100' : 'hover:bg-slate-50'}`}>
              <button onClick={() => loadConversation(conversation.id)} className="min-w-0 flex-1 px-3 py-2 text-left">
                ...
                <div className="mt-0.5 text-[10px] capitalize text-slate-400">{conversation.mode || 'assistant'}</div>
              </button>
        ...
        <header className="border-b border-slate-200 px-4 py-4 sm:px-5">
          ...
          <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
            {MODES.map((item) => {
              ...
              return (
                <button key={item.id} onClick={() => setMode(item.id)} title={item.hint} className={`flex shrink-0 items-center gap-1.5 rounded-lg border px-2.5 py-2 text-xs font-semibold transition ${mode === item.id ? 'border-[#b1191f] bg-red-50 text-[#b1191f]' : 'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-4 py-5 sm:px-6">
            ...
            <div className="mx-auto max-w-2xl py-10 text-center">
              ...
              <div className="mt-5 grid gap-2 text-left sm:grid-cols-2">
                {[
                  ...
                  'Analyse this company and identify the information we still need.',
                ].map((example) => <button key={example} onClick={() => setPrompt(example)} className="rounded-xl border border-slate-200 p-3 text-xs leading-5 text-slate-600 hover:border-slate-300 hover:bg-slate-50">{example}</button>)}
              </div>
            </div>
          )}
          ...
          {messages.map((message, index) => (
            <div key={`${message.role}-${index}`} className={`flex ${message.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[92%] rounded-2xl px-4 py-3 text-sm leading-6 sm:max-w-[82%] ${message.role === 'user' ? 'bg-[#091b2d] text-white' : 'border border-slate-200 bg-slate-50 text-slate-800'}`}>
          ...
          <div className="border-t border-slate-100 px-4 py-2 sm:px-5">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(['docx', 'pdf', 'pptx', 'xlsx'] as const).map((format) => (
                <button key={format} onClick={() => exportDraft(format)} disabled={Boolean(exporting)} className="inline-flex items-center gap-1 rounded-lg border border-slate-200 px-2.5 py-1.5 text-[10px] font-bold uppercase text-slate-600 hover:bg-slate-50 disabled:opacity-50">
                  <FileDown className="h-3 w-3" /> {exporting === format ? 'Creating…' : format}
                </button>
              ))}
            </div>
          </div>
